 ...
 * @author hoang
 */
public class SaveinfoCarOfGuestServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            HttpSession session = request.getSession(true);
             Account acc1 = (Account) session.getAttribute("name");        
            Seller s = SellerDao.getSeller(acc1.getAccID());
            ...
            if (co != false) {
             ArrayList<CarOffer> caroffer = dao.CarOfferDAO.listCarOfferByStatusSeller(2, s.getShowRoomId(), s.getSellerId());
