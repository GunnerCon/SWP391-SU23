...
import dao.CarDAO;
import dao.CarPurchaseContractDao;
import dao.OrderDAO;
 ...
 * @author hoang
 */
public class XacnhanmuaxeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String dateNotarize = request.getParameter("dateNotarize");
            int accId = 0;
            ArrayList<Order> order = new ArrayList<>();
            boolean o = CarPurchaseContractDao.insertCar(carId, sellerId, dateNotarize, dateSale, accId, orderId);
            
            if (o != false) {
                boolean car = CarDAO.ChangeStatusCar(carId, 0);              
                boolean or = OrderDAO.ChangeStatusOrder(orderId, orderId);
                if(or!=false){
